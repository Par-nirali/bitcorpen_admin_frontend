...
interface ShowArticleDetailProps {
  articleId: string | string[];
}

const ShowArticleDetail: React.FC<ShowArticleDetailProps> = ({ articleId }) => {
  const toast = useToast();
  const router = useRouter();
  const dispatch = useDispatch();
  const [showPopup, setShowPopup] = useState(false);
  const [articleDetailData, setArticleDetailData] = useState<any>(null);
  const [loading, setLoading] = useState(true);
  const modules = {
    toolbar: false,
  };
  ...
  const getArticlesDetails = async () => {
    ...
    try {
      const response = await axios({
        method: "get",
        url: `${process.env.NEXT_PUBLIC_REACT_APP_BASE_URL}/admin/article/get/${articleId}`,
        headers: { Authorization: `${token}` },
      });
  ...
  const handleLike = async () => {
    if (!articleDetailData?.latestCommets?.[0]?._id) return;
    ...
    try {
      ...
      getArticlesDetails(); // Refresh data after like
    } catch (error) {
      toast({
        title: "Error",
        description: "Failed to like post",
  ...
  if (loading) {
    return (
      <div className={styles.loadingState}>Loading article details...</div>
    );
  }

  if (!articleDetailData) {
    return <div className={styles.errorState}>Article not found</div>;
  }
          ...
          <div className={styles.graphTableDivMain}>
            ...
            <div className={styles.newsDetailDiv}>
              {articleDetailData?.mediaUrl && (
                <div className={styles.newsPointImgDiv}>
                  <img
                    src={`${process.env.NEXT_PUBLIC_REACT_APP_IMAGE_URL}/${articleDetailData.mediaUrl}`}
                    alt="article media"
                  />
                </div>
              )}
                ...
                <div className={styles.commnetUserProfImg}>
                  <img
                    src={
                      articleDetailData?.latestCommets[0]?.commentsBy
                        ?.profileImage?.url
                        ? `${process.env.NEXT_PUBLIC_REACT_APP_IMAGE_URL}/${articleDetailData.latestCommets[0].commentsBy.profileImage.url}`
                        : "/profile.svg"
                    }
                    alt="user profile"
                  />
                  ...
                  <div className={styles.commentActionsDiv}>
                    <div className={styles.commentLeftDiv}>
                      <div className={styles.ationTextDiv}>
                        ...
                        />
                        <p>
                          {articleDetailData.latestCommets[0].likeCount || 0}{" "}
                          Likes
                        </p>
                      </div>
                    </div>
